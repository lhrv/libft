...
/*   Updated: 2017/03/13 18:31:45 by lhorvat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(void)
{
	...
	list = init_list(strdup("init_on_heap"));

	list_insert_tail(list, strdup("tail_on_heap"));	
	list_insert_tail(list, (void*)malloc(1200));
	list_disp(list, 0);

	free(list_elem(list, 2)->data);
	list_elem_del(&list, 2);
	list_disp(list, 0);

	free(list_elem_del(&list, 1));
	list_disp(list, 0);
